...
from kivy.properties import StringProperty, BooleanProperty #! Import to customize properties in the kivy document.
...
class WidgetsExample(GridLayout):
    the_count = 0
    count_enabled = BooleanProperty(False) #! This is to determine what state the toggle button is in.
    my_text = StringProperty(str(the_count)) #?have to create a custom property
    #slider_value_text = StringProperty(str("Value"))
    ...
    def on_slider_value(self, widget):
        print("Slider: " + str(int(widget.value))) # The  value is a float. Use str to convert to text so it will output, then int to convert to whole  numbers. 
...
class slider_exerciseApp(App):
    pass

slider_exerciseApp().run()
